 ...
 */
export const InspectionVirtualizer: Aspect<InspectionAspectData> = {
    name: "globSprayer",
    displayName: undefined,
    extract: async () => [],
    consolidate: async fingerprints => {
        const emitted: Array<FP<InspectionAspectData>> = [];
        const projectPaths = distinctNonRootPaths(fingerprints);
        const globFingerprints = fingerprints.filter(isInspectionFingerprint);
        for (const path of projectPaths) {
            for (const gf of globFingerprints) {
                const data = {
                    ...gf.data,
                    matches: gf.data.comments
                        .filter(c => c.sourceLocation)
                        .filter(c => c.sourceLocation.path.startsWith(path)),
                };
                emitted.push({
                    ...gf,
                    data,
